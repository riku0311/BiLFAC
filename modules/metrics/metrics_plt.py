...
def save_unique_plot(filename, folder='imgs/plots2'):
    # フォルダが存在しない場合は作成
...
if __name__ == "__main__":
    ...
    end_index=120
    data_num=47
    datas=list(range(1,48))
    gops=[5,10,15]
    frame_indices = list(range(1,end_index+2))

    # csv_file_path = '/home/21x3109_takahashi/hdac/results/R2HDAC/additional_wo_enh_results.csv'
    # input_csv_file = '/home/21x3109_takahashi/hdac/results/R2HDAC/R2HDAC_wo_enh_results.csv'
    # output_csv_file = '/home/21x3109_takahashi/hdac/results/R2HDAC/wo_enh_results.csv'
    # header = ['model','deta_num','GOP','psnr','lpips', 'dists']


    # hdac, w/o down&enh, w/o enh, oursのPSNRを計算し最もoursのPSNRが高いフレームを取得
    # if 
    # ours_metrics['psnr'][frame_idx] > wo_down_enh_metrics['psnr'][frame_idx] and /
    # ours_metrics['psnr'][frame_idx] > wo_enh_metrics['psnr'][frame_idx] and /
    # wo_down_enh_metrics['psnr'][frame_idx] > wo_enh_metrics['psnr'][frame_idx] and /
    # not frame_idx%gop==0
    # psnr_csv_file_path = '/home/21x3109_takahashi/hdac/results/R2HDAC/psnr_results.csv'

    # if 
    # ours_metrics['psnr'][frame_idx] > wo_down_enh_metrics['psnr'][frame_idx] and /
    # ours_metrics['psnr'][frame_idx] > wo_enh_metrics['psnr'][frame_idx] and /
    # ours_metrics['psnr'][frame_idx] > hdac_metrics['psnr'][frame_idx] and /
    # not frame_idx%gop==0:
    psnr_csv_file_path2 = '/home/21x3109_takahashi/hdac/results/R2HDAC/psnr_results2.csv'
    
    with open(psnr_csv_file_path2, mode='w', newline='') as file:
        writer = csv.writer(file)
    
        # Write the header
        writer.writerow(['Data Num', 'GOP','Frame Index', 'HDAC PSNR', 'w/o down&enh PSNR', 'w/o enh PSNR', ' Ours PSNR', 'max difference'])
        
        for idx in tqdm(range(1,data_num+1)):
            original_path= f'/home/21x3109_takahashi/hdac/BFDAC_testset/data{idx}.mp4'
            org = imageio.mimread(original_path, memtest=False)[start_index:end_index+1]

            for gop in gops:
                # 動画のリスト
                hdac_path= f'/home/21x3109_takahashi/hdac/results/HDAC/video/HDAC_data{idx}_{gop}_30.mp4'
                wo_down_enh_path= f'/home/21x3109_takahashi/hdac/results/R2HDAC/wo_down_enh_video/R2HDAC_data{idx}_{gop}_30.mp4'
                wo_enh_path= f'/home/21x3109_takahashi/hdac/results/R2HDAC/wo_enh_video/R2HDAC_data{idx}_{gop}_30.mp4'
                ours_path= f'/home/21x3109_takahashi/hdac/results/R2HDAC/video/R2HDAC_data{idx}_{gop}_30.mp4'

                hdac = imageio.mimread(hdac_path, memtest=False)
                wo_down_enh = imageio.mimread(wo_down_enh_path, memtest=False)
                wo_enh = imageio.mimread(wo_enh_path, memtest=False)
                ours = imageio.mimread(ours_path, memtest=False)

                hdac_metrics = metrics.compute_metrics(org, hdac, metrics=['psnr'])
                wo_down_enh_metrics = metrics.compute_metrics(org, wo_down_enh, metrics=['psnr'])
                wo_enh_metrics = metrics.compute_metrics(org, wo_enh, metrics=['psnr'])
                ours_metrics = metrics.compute_metrics(org, ours, metrics=['psnr'])

                max_difference = 0
                max_diff_frame_idx = -1
                for frame_idx in range(len(frame_indices)):
                    if (ours_metrics['psnr'][frame_idx] > wo_down_enh_metrics['psnr'][frame_idx] and 
                        ours_metrics['psnr'][frame_idx] > wo_enh_metrics['psnr'][frame_idx] and 
                        ours_metrics['psnr'][frame_idx] > hdac_metrics['psnr'][frame_idx] and not frame_idx%gop==0):

                        difference = (abs(ours_metrics['psnr'][frame_idx] - wo_down_enh_metrics['psnr'][frame_idx]) \
                                      + abs(ours_metrics['psnr'][frame_idx] - wo_enh_metrics['psnr'][frame_idx]) \
                                      + abs(ours_metrics['psnr'][frame_idx] - hdac_metrics['psnr'][frame_idx])) / 3
                        
                        if difference > max_difference:
                            max_difference = difference
                            max_diff_frame_idx = frame_idx

                if max_diff_frame_idx != -1:
                        writer.writerow([idx, gop, max_diff_frame_idx, 
                                        hdac_metrics['psnr'][max_diff_frame_idx], 
                                        wo_down_enh_metrics['psnr'][max_diff_frame_idx], 
                                        wo_enh_metrics['psnr'][max_diff_frame_idx],
                                        ours_metrics['psnr'][max_diff_frame_idx], 
                                        max_difference])
    

    #　HDACの各データのPSNRの平均値を計算
    # psnr_csv_file_path='/home/21x3109_takahashi/hdac/results/HDAC/psnr_results.csv'        
    # with open(psnr_csv_file_path, mode='w', newline='') as file:
    #     writer = csv.writer(file)
    #     for data in tqdm(datas):
    #         org = imageio.mimread(f'/home/21x3109_takahashi/hdac/BFDAC_testset/data{data}.mp4', memtest=False)[start_index:end_index+1]

    #         for gop_size in gops:
    #             hdac=imageio.mimread(f'/home/21x3109_takahashi/hdac/results/HDAC/video/HDAC_data{data}_{gop_size}_30.mp4', memtest=False)
    #             hdac_metrics = metrics.compute_metrics(org, hdac, metrics=['psnr'])
    #             hdac_psnr = np.mean(hdac_metrics['psnr'])
    #             writer.writerow([hdac_psnr])
    

    # df1 = pd.read_csv(input_csv_file)
    # df2 = pd.read_csv(csv_file_path)
    # df2['bpp']=df1['bpp']
    # df2['bitrate']=df1['bitrate']
    ...
    #     data={
    #         'gop_size':gop_size,
    #         'psnr_avg_hdac':psnr_avg_hdac.tolist(),
    #         'psnr_avg_ours':psnr_avg_r2hdac.tolist(),
    #         'lpips_avg_hdac':lpips_avg_hdac.tolist(),
    #         'lpips_avg_ours':lpips_avg_r2hdac.tolist(),
    #         'dists_avg_hdac':dists_avg_hdac.tolist(),
    #         'dists_avg_ours':dists_avg_r2hdac.tolist()
    #     }
    ...
    # with open('/home/21x3109_takahashi/hdac/results/quality_comparison_across_frames.json', 'w') as file:
