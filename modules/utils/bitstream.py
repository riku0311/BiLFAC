...
class KeypointCompress:
    ...
    def _decode_kp_val(self, src: Dict[str, torch.tensor], kp_res: List[int])-> torch.tensor:
        kp_res = torch.tensor(np.array(kp_res).reshape(1, self.num_kp, 2), dtype=torch.float32,device=src.device)
        sr = torch.round((src+1)*self.q_step/1).int()
        dr = (sr - kp_res)/self.q_step - 1
        return dr

    def _decode_kp_jacobians(self,src: Dict[str, torch.tensor], kp_res: List[int])-> torch.tensor:
        kp_res = torch.tensor(np.array(kp_res).reshape(1, self.num_kp, 2,2), dtype=torch.float32,device=src.device)
        dr = (src - kp_res)/self.q_step - 1
        return dr

    def encode_kp(self, kp_prev: Dict[str, torch.Tensor], kp_driving: Dict[str, torch.Tensor]) -> Tuple[Dict, torch.Tensor]:
        ...
        # use temp dir instead of fixed out_dir
        with tempfile.TemporaryDirectory() as tmpdir:
            out_path = os.path.join(tmpdir, f'{self.idx}.bin')
            final_encoder_expgolomb(res, out_path, 0)
            self.total_bytes += filesize(out_path)
            # tmpdir and out_path will be auto-deleted
